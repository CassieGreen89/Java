...
//	Remove the odd numbers.
	public void exerciseFive() {
		List<Integer> evens = exThree.stream().filter(x -> x % 2 == 0).collect(Collectors.toList());
		System.out.println(evens);
	}

//	Remove the even numbers.
	public void exerciseSix() {
		List<Integer> odds = exThree.stream().filter(x -> x % 2 != 0).collect(Collectors.toList());
...
//	find the min value. Map, then filter, then min
	public void exerciseEight() {
		Integer complexStream = exThree.stream()
										.map(a -> a * a)
										.filter(x -> x % 2 != 0)
